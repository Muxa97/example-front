<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.volumeBTC | 0.0 }} BTC</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.profitBTC | 0.0 }} BTC</span>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
<script lang="ts">
  ...
  export default class extends Vue {
        ...
        this.analyzedExchangeDayGroups.push(
          {
            ...
            volumeBTC: data.transactions.reduce((acc: number, tx: any) => acc + (+tx.usdValue / BTC.PRICE), 0),
            profitBTC: data.transactions.reduce((acc: number, tx: any) => acc + (tx.status === 'finished' ? (+tx.usdValue / BTC.PRICE) : 0), 0)
          }
        )
    ...
    private redirectToWaitingExchanges() {
      this.$router.push('/exchange/waiting')
    }
  }
</script>
